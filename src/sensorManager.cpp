...
void monitorSensors()
{
    ...
    {
        quickBlinkAndHalt(); // Stop the system when the limit is reached
        return;
    }
    ...
    for (int phase = 1; phase <= CYCLES_FOR_5_MIN; phase++)
    {
        ...
        while (!phaseCompleted && retryCount < 3)
        {
            ...
            {
                phaseCompleted = true;
                powerCycleMPU(false); // Power off the MPU after phase completes
            }
            else
            {
...
{
    toggleSensorPower(on); // Use Vext to power on/off the MPU
